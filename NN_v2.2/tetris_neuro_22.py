...
def main(win, net):
    ...
    while run:
        ...
        # PIECE FALLING CODE
        if fall_time/1000 >= evolution.fall_speed:
            fall_time = 0
            current_piece.y += 1
        ...
        net.layers[0].forward(net.inputs)
        net.layers[0].activation_ReLU(net.layers[0].output)
        
        net.layers[1].forward(net.layers[0].output)
        net.layers[1].activation_ReLU(net.layers[1].output)
        
        net.layers[2].forward(net.layers[1].output)
        net.layers[2].activation_sigmoid(net.layers[2].output)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_b:
                    evolution.fall_speed = 0.1
                    
                if event.key == pygame.K_n:
                    evolution.fall_speed = 0.01
                    
                if event.key == pygame.K_b:
                    evolution.fall_speed = 0.001
    ...
    print(f"Sc: {score}, pcs: {score_of_fallen_pieces}, ", end="", flush=True)
    print(f"lvl: {level}")
...
evolution.number_of_agents = 20

evolution.number_of_generations = 100
...
def animate(i):
    y_axis = []
    for generation in evolution.list_of_all_generations:
    ...
    plt.plot(x_axis, y_axis, label='Channel 1')
...
for generation in range(evolution.number_of_generations):
    ...
    for index, net in enumerate(evolution.list_of_neural_nets):
        ...
        # Play many times to see if good or not
        for _ in range(1):
            play_tetris(net)
        ...
        print(f"Total score: {net.get_score()[-1]}")
