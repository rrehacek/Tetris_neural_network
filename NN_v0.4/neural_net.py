...
class NeuralNetwork():
    def __init__(self):
        ...
        self.weights_left = 0.010 * np.random.randn(200, 1)
        self.weights_down = 0.010 * np.random.randn(200, 1)
        self.weights_right = 0.010 * np.random.randn(200, 1)
        self.weights_rotate = 0.010 * np.random.randn(200, 1)
    ...
    def forward_activation_function(self):
        ...
        self.out_left = 1 if in_left > 0.3 or in_left < -0.3 else 0
        self.out_down = 1 if  in_down > 0.3 or in_down < -0.3 else 0
        self.out_right = 1 if in_right > 0.3 or in_right < -0.3 else 0
        self.out_rotate = 1 if in_rotate > 0.3 or in_rotate < -0.3 else 0
