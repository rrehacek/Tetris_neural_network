...
class EvolutionAlgorithm():
    ...
    def populate_list_of_neural_nets(self):
        for _ in range(self.number_of_agents):
            self.list_of_neural_nets.append(NeuralNetwork(210, self.number_of_neurons))
    ...
    def crossover(self):
        ...
        while len(self.new_generation) < self.number_of_agents:
            ...
            new_agent = NeuralNetwork(210, self.number_of_neurons)
            
            random_place_in_genom = random.randint(1, 209)
    ...
    def mutate(self):
        ...
        for i, agent in enumerate(self.new_generation):
            ...
            if not i < 2 and i % 2 == 0:
                agent.weights += 0.05 * np.random.randn(210, 1)
                agent.out_weights += 0.05 * np.random.randn(len(agent.out_weights), 1)
