...
for generation in range(number_of_generations):
    ...
    sorted_by_level = sorted(list_of_neural_nets, key=lambda x: x.level_score, reverse=True)
    for net in sorted_by_level:
        if net.level_score > 0:
            new_generation.append(net)
    
    
    sorted_by_pieces = sorted(list_of_neural_nets, key=lambda x: x.piece_score, reverse=True)
    if len(new_generation) < number_of_agents:
        for net in sorted_by_pieces:
            if net.piece_score > sorted_by_pieces[int(len(sorted_by_pieces)/2)].piece_score:
                new_generation.append(net)
    
    
    #To fulfill the capacity of list, mutate current best agents and add them as new ones
    while len(new_generation) < number_of_agents:
        
        #Create new agent and mutate its weights
        try:
            new_agent = new_generation[0].copy()
        except Exception as exception:
            new_agent = NeuralNetwork()
            print(f"Exception occured: {exception}")
            print(f"New agent had to be created.")
        new_agent.weights_left + 0.01 * np.random.randn(200, 1)
        new_agent.weights_down + 0.01 * np.random.randn(200, 1)
        new_agent.weights_right + 0.01 * np.random.randn(200, 1)
        new_agent.weights_rotate + 0.01 * np.random.randn(200, 1)
        
        new_generation.append(new_agent)
    
    list_of_neural_nets = []
    list_of_neural_nets = new_generation
...
# [n.get_score() for n in sorted(list_of_neural_nets, key=lambda x: x.level_score, reverse=True)]
