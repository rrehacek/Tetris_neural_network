...
def main(win, net):
    ...
    while run:
        ...
        if nn_key_outputs[0] > 0:
            current_piece.x -= 1
            if not valid_space(current_piece, grid):
                current_piece.x += 1
        
        if nn_key_outputs[1] > 0:
            while valid_space(current_piece, grid):
                current_piece.y += 1
            current_piece.y -= 1
            
        if nn_key_outputs[2] > 0:
            current_piece.x += 1
            if not valid_space(current_piece, grid):
                current_piece.x -= 1
         
        if nn_key_outputs[3] > 0:
    ...
    print(f"pieces: {score_of_fallen_pieces}, ", end="", flush=True)
    print(f"level: {level}")
...
evolution.number_of_agents = 6

evolution.number_of_generations = 15
