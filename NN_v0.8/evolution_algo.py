...
class EvolutionAlgorithm():
    ...
    def crossover(self):
        ...
        while len(self.new_generation) < self.number_of_agents:
            ...
            new_agent.weights = np.concatenate((self.list_of_neural_nets[i].weights[:int(len(self.list_of_neural_nets[i].weights)/2)], \
                                                     self.list_of_neural_nets[i+1].weights[int(len(self.list_of_neural_nets[i+1].weights)/2):]))
